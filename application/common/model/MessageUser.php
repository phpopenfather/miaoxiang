...
class MessageUser extends Model
{
    ...
    {
        return $this->belongsTo('User', 'send_id', 'id', [], 'LEFT')->setEagerlyType(0);
    }
     ...
     * @example 示例
     */
    public function getList($user_id, $type = '', $is_see=false)
    {
        ...
        $list = $this->with(['messagenotice','senduser'])->where($where)->order('createtime DESC')->paginate(10);
        foreach ($list as $row) {
            $row->visible(['rec_id','user_id','message_id','is_see','deleted','createtime']);
            $row->visible(['messagenotice']);
            $row->getRelation('messagenotice')->visible(['message_type', 'message_title', 'message_content','message_annex','lable','type', 'extraction_code']);
            $row->visible(['senduser']);
            $row->getRelation('senduser')->visible(['id','username', 'mobile']);
        }
        return $list;
    }
